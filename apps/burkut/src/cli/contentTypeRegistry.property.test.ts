...
const arbMixedCase = (ext: string) =>
  fc.array(fc.boolean(), { minLength: ext.length, maxLength: ext.length }).map((flags) =>
    ext
      .split("")
...
describe("Content Type Registry — Property 3: Content type extension mapping", () => {
  ...
  it("returns the correct ContentType for any registered extension in any case", () => {
    fc.assert(
      fc.property(
        fc
          .constantFrom(...REGISTERED_EXTENSIONS)
          .chain(({ ext, type }) =>
            fc.tuple(arbDirPrefix, arbFilename, arbMixedCase(ext), fc.constant(type)),
          ),
