...
export function scanDirectory(rootPath: string): ScannedFile[] {
  ...
  function walk(dir: string): void {
    // Typed against `Dirent[]` (string names) rather than inferred from the
    // overloaded `readdirSync` signature: `ReturnType<typeof readdirSync>`
    // picks the Buffer overload under @types/node 25. Type-only distinction.
    let entries: Dirent[];
    try {
