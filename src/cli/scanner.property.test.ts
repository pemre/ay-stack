...
describe("Scanner — Property 1: Scan completeness and exclusion", () => {
    ...
    it("returns exactly the recognized, non-hidden, non-node_modules files", () => {
        fc.assert(
            fc.property(
                fc.record({
                    recognizedFiles: fc.array(
                        fc.tuple(arbStem, fc.constantFrom(...RECOGNIZED_EXTENSIONS)),
                        { minLength: 1, maxLength: 6 },
                    ),
                    hiddenFiles: fc.array(
                        fc.tuple(arbStem, fc.constantFrom(...RECOGNIZED_EXTENSIONS)),
        ...
        fc.assert(
            fc.property(
                fc.record({
                    ...
                    nonMdFiles: fc.array(
                        fc.tuple(arbStem, fc.constantFrom(...NON_MARKDOWN_EXTENSIONS)),
                        { minLength: 1, maxLength: 4 },
                    ),
                }),
                ({ mdFiles, nonMdFiles }) => {
