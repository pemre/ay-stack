...
| --- | --- | --- | --- | --- |
| **Core** | `--{category}-{name}-{scale}` | `--color-amber-500`, `--space-2`, `--radius-md`, `--duration-fast` | `@ay/ui-library` — `src/tokens/core.css` | the semantic tier only |
| **Semantic** | `--{category}-{context}` | `--color-primary`, `--color-bg-surface`, `--color-border-default`, `--radius-control` | `@ay/ui-library` — `src/tokens/semantic.css` | component CSS, application CSS |
...
1. `@ay/ui-library` owns the core and semantic tiers, and only those. It declares no
...
- The suffix convention is internally inconsistent and deliberately preserved:
  ...
  `0x66` = 0.400). Normalizing would change rendered alpha by up to 1/255.
   `src/tokens/core.css` documents this in place.
