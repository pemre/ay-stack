...
//
// This is the @ay/ui-library token half. It quantifies over every CSS file both consumers
...
/** Names the token sources must never declare, because another tier owns them. */
const APP_SPECIFIC_PREFIXES = ["--tl-bg-", "--vis-"];
const APP_SPECIFIC_NAMES = ["--font-serif"];
const COMPONENT_PREFIXES = ["--spiral-", "--image-zoom-", "--btn-"];

const CONSUMER_DIRS = [
  join(ROOT, "packages", "ui-library", "src", "blocks"),
  join(ROOT, "packages", "ui-library", "src", "dashboard"),
...
const CORE = new Set(declaredNames(read(PKG, "src", "tokens", "core.css")));
const SEMANTIC_CSS = read(PKG, "src", "tokens", "semantic.css");
...
describe("Property 3: bidirectional tier ownership — @ay/ui-library token half", () => {
  it("declares only core-tier and semantic-tier names", () => {
    const declaredByPackage = cssFiles(join(PKG, "src", "tokens")).flatMap((file) =>
    ...
    expect(foreign, "token sources declare a name in neither owned tier").toEqual([]);
    ...
    expect(trespassing, "token sources declare a legacy alias, app, or component token").toEqual(
      [],
    );
  ...
  it("finds no owned token declared by either consumer", () => {
    ...
    expect(violations, "a consumer declares a token source-owned value").toEqual([]);
